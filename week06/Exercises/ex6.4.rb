...
# Seu código aqui...
campos = str.scan(/^(\d{2}.\d{2}) - \[(.*)\] (.*).*$/)
campos.length.times do |i|
  puts "#{campos[i][1]}: #{campos[i][2]} (#{campos[i][0]})".upcase
end

# 2) Quantas linhas existem nesta string?

# Seu código aqui...
puts "Existem #{campos.length} linhas."

# 3) Quantas notícias de economia de falam sobre o dólar?

# Seu código aqui...
dolar = str.scan(/(dólar)/)
puts "#{dolar.length} notícias falam sobre o dólar."

# 4) Imprimir todos os horários das notícias que contém a palavra golfe.
golfe = str.scan(/(\d{2}.\d{2}).*golfe/)
puts "#{golfe}"
